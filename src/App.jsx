...
export default function App() {
  ...
  const handleAddProduct = () => {
    ...
    setProducts([
      ...products,
      { name: newProductName, price: parseInt(newProductPrice) },
    ]);
    setNewProductName("");
    setNewProductPrice("");
  };
  ...
  const handleUpdateProductPrice = (index, newPrice) => {
    const updated = [...products];
    updated[index].price = parseInt(newPrice);
    setProducts(updated);
  };
          ...
          {products.map((p, i) => (
            <li key={i} className="flex justify-between items-center gap-2">
              ...
              <input
                type="number"
                value={p.price}
                onChange={(e) => handleUpdateProductPrice(i, e.target.value)}
